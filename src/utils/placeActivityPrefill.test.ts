...
test('obvious place categories suggest an editable activity category', () => { assert.equal(activityCategoryForPlace('playground'), 'playground_meetup'); assert.equal(activityCategoryForPlace('library'), 'story_time'); assert.equal(activityCategoryForPlace('other'), 'other'); });
