...
async function createFindingApprovals(store: ScheduledOperatorStore, runId: string, findings: OperatorFinding[], pending: Set<string>, now: Date, idFactory: () => string): Promise<ScheduledCycleReport['approvalsCreated']> {
  ...
  for (const finding of visibleQueue(findings).filter((item) => item.autonomy === 'YELLOW' && (item.priority === 'P0' || item.priority === 'P1')).slice(0, 3)) {
    ...
    const providerIssue=finding.domain==='provider';
    const key = `${providerIssue?'provider_health':'content_quality'}:${finding.id}`;
    if (pending.has(key)) continue;
    const request: OwnerApprovalRequest = { approvalId:idFactory(), runId, category:providerIssue?'provider_health':'production_content_correction', title:finding.title, summary:finding.evidence, whyNow:`Detected during the scheduled operator cycle at priority ${finding.priority}`, recommendedAction:providerIssue?'Investigate the provider state and prepare a dry-run recovery plan; do not change its cron or production state':'Review the affected records and approve a scoped correction plan', riskLevel:finding.priority==='P0'?'high':'medium', evidence:{findingId:finding.id,priority:finding.priority,evidence:finding.evidence}, dryRunResults:{productionWrites:0}, expectedImpact:providerIssue?'Restore reliable provider coverage without interpreting a failed or incomplete source as deletion':'Remove misleading or low-quality content without changing unrelated records', rollbackPlan:providerIssue?'No provider setting or schedule is changed before approval':'No production correction is executed until approval; any approved change must include record-level rollback SQL', agentRecommendation:'APPROVE', createdAt:now.toISOString(), status:'PENDING' };
