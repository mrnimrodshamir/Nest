-- PREPARED ONLY. Do not run until a controlled Weekend push is physically
-- validated and the owner explicitly approves production enablement.
do $$
declare
  v_job record;
...
begin
  ...
  for v_job in select jobid from cron.job
    where jobname = 'send-weekend-digest-jerusalem-1800'
       or command like '%send-daily-digest%weekend%'
  loop perform cron.unschedule(v_job.jobid); end loop;
  perform cron.schedule(
    'send-weekend-digest-jerusalem-1800',
    '*/15 * * * *',
    format($command$
...
-- select cron.unschedule('send-weekend-digest-jerusalem-1800');
