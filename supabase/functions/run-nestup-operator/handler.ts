...
export interface OperatorHandlerDependencies { serviceRoleKey:string; client:SupabaseClient; now?:()=>Date }
...
export async function handleOperatorRequest(request:Request,deps:OperatorHandlerDependencies):Promise<Response>{
  ...
  if(!token||token!==deps.serviceRoleKey)return response(401,{error:{code:'UNAUTHORIZED'}});
