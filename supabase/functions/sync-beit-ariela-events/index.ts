...
import { fetchBeitArielaCandidates } from '../_shared/beitAriela/connector.ts';
import { mapBeitArielaRecord, BEIT_ARIELA_PROVIDER_KEY, BEIT_ARIELA_SOURCE_NAME, BEIT_ARIELA_PROVIDER_URL } from '../_shared/beitAriela/mapping.ts';
...
Deno.serve(async (request: Request) => {
  ...
  try {
    const outcome = await runGenericProviderSync({
      providerKey: BEIT_ARIELA_PROVIDER_KEY,
      sourceName: BEIT_ARIELA_SOURCE_NAME,
      providerUrl: BEIT_ARIELA_PROVIDER_URL,
      fetchCandidates: async () => {
        const fetchResult = await fetchBeitArielaCandidates({ horizonDays: 7 });
        const candidates = fetchResult.records.map((r) => mapBeitArielaRecord(r)).filter((row) => row.candidate).map((row) => row.candidate);
