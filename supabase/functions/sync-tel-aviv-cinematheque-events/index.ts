...
import { fetchCinemathequeCandidates } from '../_shared/cinematheque/connector.ts';
import { mapCinemathequeOccurrence, CINEMATHEQUE_PROVIDER_KEY, CINEMATHEQUE_SOURCE_NAME, CINEMATHEQUE_PROVIDER_URL } from '../_shared/cinematheque/mapping.ts';
...
Deno.serve(async (request: Request) => {
  ...
  try {
    const outcome = await runGenericProviderSync({
      providerKey: CINEMATHEQUE_PROVIDER_KEY,
      sourceName: CINEMATHEQUE_SOURCE_NAME,
      providerUrl: CINEMATHEQUE_PROVIDER_URL,
      fetchCandidates: async () => {
        const fetchResult = await fetchCinemathequeCandidates({ horizonDays: 7 });
        const candidates = fetchResult.occurrences.map((r) => mapCinemathequeOccurrence(r)).filter((row) => row.candidate).map((row) => row.candidate);
        return { candidates, sourceComplete: fetchResult.sourceComplete, incompleteReason: fetchResult.incompleteReason, rawCount: fetchResult.rawCardCount };
      },
    }, database);
