-- Forward-only production reconciliation of reviewed local migration 0005.
...
begin
  if not exists (
    ...
    where conrelid = 'public.activities'::regclass
      and conname = 'activities_place_provider_check'
  ) then
    alter table public.activities
  ...
  if not exists (
    select 1 from pg_constraint
    where conrelid = 'public.activities'::regclass
      and conname = 'activities_location_source_check'
  ) then
    alter table public.activities
